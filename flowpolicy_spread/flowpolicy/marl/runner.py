...
class Runner:
    def __init__(self, args, env):
        ...
        header = [
            "episode", "reward", "q_actor", "q_mean", "q_max", "q_min",
    ...
    def run(self):
        ...
        # self.noise = max(0.05, self.noise * 0.9995)
        # self.epsilon = max(0.05, self.epsilon * 0.999) # 调整noise与epsilon
        self.plot_rewards()
    ...
    def plot_rewards(self):
        ...
        if len(rewards) >= window:
            ...
            plt.plot(shaped_avg, label='Shaped Reward (MA 50)')
            plt.plot(true_avg, label='True Reward (MA 50)')
        else:
            plt.plot(rewards, label='Shaped Reward', alpha=0.3)
            plt.plot(true_rewards, label='True Reward', alpha=0.3)
        ...
        print(f"奖励曲线已保存到 training_reward_curve.png")
